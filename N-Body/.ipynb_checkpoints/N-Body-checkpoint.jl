...
let G=1.0e-3
    "Generic Gravity, intakes two bodies, outputs the magnitude (and sign) of gravity"
...
"Move 1 timestep forward, using a symplectic integrator to conserve energy"
function step!(sim::NBodySim)
    ...
    #Now use leapfrog integration to step the planets about, and then archive the old accelerations for the next leapfrogging
    for bod in sim.bodies
